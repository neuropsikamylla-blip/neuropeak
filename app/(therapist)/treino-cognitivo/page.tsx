  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Treino Cognitivo</h1>
          <p className="text-sm text-slate-400">Domínios cognitivos e os exercícios de cada um</p>
        </div>
      </div>
      ...
      <section className="bg-[#14264e] border border-white/10 rounded-[24px] p-8 shadow-[0_1px_3px_rgba(0,0,0,0.2),0_10px_30px_-16px_rgba(0,0,0,0.45)]">
            ...
            return (
              ...
              >
                ...
                <span
                  ...
                  style={isActive ? { backgroundColor: color } : { backgroundColor: `${color}26` }}
                >
                  <ChevronDown
                ...
                <span className="block w-16 h-px bg-white/10 my-4" />
                <p className="text-sm text-slate-400 leading-relaxed">{DOMAIN_DESCRIPTIONS[domain]}</p>
              </button>
            );
          })}
        </div>
        ...
        {active && (
          <div
            ...
            style={{ borderColor: `${DOMAIN_COLORS[active]}44`, backgroundColor: `${DOMAIN_COLORS[active]}1A` }}
          >
            ...
            <div className="space-y-5">
              {DOMAIN_SUBDOMAINS[active].map((sd) => {
                ...
                return (
                  <div key={sd.id}>
                    <p className="text-[11px] font-bold uppercase tracking-wide text-slate-400 mb-2 pl-1 flex items-center gap-2">
                      {sd.label}
                      {empty && <span className="text-[10px] italic font-normal text-slate-500 normal-case tracking-normal">em breve</span>}
                    </p>
                    {empty ? (
                      <div className="rounded-xl border border-dashed border-white/15 bg-white/5 px-3 py-2.5">
                        <p className="text-xs text-slate-400">Subdomínio em construção — novos exercícios em breve.</p>
                      </div>
                    ) : (
