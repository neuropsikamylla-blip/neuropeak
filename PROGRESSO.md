...
plataforma está no bloco de 14/set, acima.
...
- [x] **Passo A1 — campo de ordem no dado, sem renomear imagem.** `ord` opcional em `HistoriaDef`,
      ...
      **Provado:** os dois gabaritos conferidos pelo VP **nas próprias imagens**, uma a uma; suíte de
      `lib/ordem-historia` verde; nenhum arquivo de imagem no diff. **Por injeção:** voltar a imagem
      para `order + 1`, apagar o `ord` de `f12` e comparar o **painel** em vez da **posição** —
      **cada uma derruba os testes**.
- [x] **Passo A2 — `d8` sai do sorteio.** Marcado `duplicataDe: "d2"` (é a mesma história) e fora do
      pool. *Critério:* teste que conta as histórias sorteáveis e afirma que `d8` **não** aparece.
      **Provado:** teste de contagem do pool verde; **por injeção**, remover o filtro de duplicata
      derruba o teste.
- [x] **Passo A3 — embaralhamento que não se resolve com uma troca.** Novo
      ...
      **Provado:** a prova por volume rodou e passou nos quatro tamanhos, com a permutação íntegra em
      todas as execuções.
- [x] **Passo A4 — a linha técnica sai da interface.** "Nível 1 · 4 cenas · fácil" some em **dois**
      ...
      **Provado:** contagem zero nos quatro literais; **por injeção**, reinserir a linha técnica
      derruba o teste — a prova é de ausência, como o critério exigia.

**Fatia B — a segunda tentativa e o toque.** Spec em `b1220c26`. Entregue em `a040176e` (**v3.28.0**).

- [x] **Passo B1 — segunda tentativa com os verdes mantidos e SEM travar.** *Critério:* teste do
      ...
      **Provado:** fase `corrigindo` com os cartões livres; a marca verde é **derivada da posição
      atual** e some sozinha se o cartão sair de lá. **Por injeção:** travar os cartões na 2ª
      tentativa derruba os testes.
- [x] **Passo B2 — registro da 1ª tentativa separado no `metadata`.** *Critério:* teste do payload
      ...
      **Provado:** a acurácia gravada é a da **PRIMEIRA** confirmação; 6 campos novos no `metadata`
      (`storiesFirstTryExact`, `storiesSolvedAfter`, `storiesUnsolved`, `accFirstTry`, `accFinal`,
      `confirmationsTotal`), com `accFinal` **apenas informativo**. **Por injeção:** gravar a nota
      final em vez da primeira derruba os testes. É a mesma armadilha que **inflou a Torre em 31/ago
      e a Grade Dedutiva em 03/set**.
- [x] **Passo B3 — o retorno visual do arraste.** ⚠️ **A auditoria errou aqui, e o VP corrigiu o
      documento em 14/set (`443004a8`):** o cartão **inteiro já arrasta** (`OrdemHistoria.tsx:127`
      põe os `listeners` no container; o `⠿` era `aria-hidden`, desenho sem listener). O que faltava
      não era mecânica, era **comunicação**: o ícone de alça dizia ao paciente que só ali se arrasta.
      *Critério:* teste que prova que os `listeners` seguem no container (prova por posição, não por
      presença) + verificação visual dela.
      **Provado:** ícone `⠿` **removido**; teste de posição confirma os `listeners` no container.
      ⏳ **A verificação visual dela continua pendente** (ver seção vermelha abaixo).
- [x] **Passo B4 — conclusão visual curta.** *Critério:* teste do contrato da tela + verificação dela.
      **Provado:** conclusão **"Sequência correta."**, sem confete; teste de contrato verde.
      ⏳ Verificação dela pendente.
- [x] **Passo B5 — layout e estados do botão Confirmar.** *Critério:* teste dos estados (desabilitado
      enquanto falta ordenar, rótulo distinto na 2ª tentativa) + verificação dela.
      **Provado:** estados testados e rótulo **"Confirmar de novo"** na 2ª tentativa. ⏳ Verificação
      dela pendente.
- [x] **Passo B6 — tutorial só na primeira utilização.** *Critério:* o tutorial não reaparece na
      segunda sessão do mesmo paciente.
      **Entregue com limitação declarada:** o tutorial completo aparece **só na 1ª vez**, por
      `localStorage` (`np-ordem-historia-visto`) — ou seja, **por APARELHO, não por paciente**,
      porque Ordem da História ainda não foi convertida ao framework T1. Também ficaram persistidas
      as histórias recentes (`np-ordem-historia-recentes`), que antes viviam num `useRef` e se
      perdiam ao recarregar. A decisão sobre o T1 segue aberta — ver seção vermelha.
...
| commit | o que é |
|---|---|
| `443004a8` | correção da auditoria — o cartão inteiro já arrastava; o ícone de alça é que enganava |
| `c3333e1b` | spec da Fatia A |
| `b1220c26` | spec da Fatia B |
| `513aa1a8` | bloco EM ANDAMENTO no PROGRESSO |
...
**Os quatro consertos do VP sobre a entrega do Codex (Fatia B), todos defeitos reais:**

1. **`posCorrect`/`posWrong` somavam a cada confirmação** — a métrica que o terapeuta lê saía
   **inflada** a cada tentativa extra.
2. **`setTimeout` de 120 ms do botão sem limpeza ao desmontar.**
3. **Os 30 ids recentes vetavam a faixa inteira** (20 a 22 histórias) e tornavam o "não repetir"
...
1. **A verificação visual dela em produção** — nada substitui. O que conferir, jogando:
   - o **cinema** (`f12`) e o **herbário** (`d20`) na ordem certa;
   - a **segunda tentativa**: o verde que aparece, o cartão que continua podendo sair do lugar, e o
     botão "Confirmar de novo";
   - a tela **sem a linha técnica** ("Nível 1 · 4 cenas · fácil") nos dois lugares;
   - o **arraste no celular**, agora sem o ícone `⠿` — é lá que o toque decide se ficou claro.
2. **A classificação das 86 histórias por demanda cognitiva e por tipo de raciocínio** (seções 4 e 14
   da espec dela). **Trabalho clínico dela, não de código:** nenhum algoritmo decide qual história
   exige mais inferência causal. **Sem essa classificação, a dificuldade continua andando só por
   número de cenas.**
3. **A decisão sobre converter Ordem da História ao framework T1.** Hoje o tutorial de primeira
   utilização vive em `localStorage` — **por aparelho, não por paciente**: quem trocar de celular vê
   o tutorial de novo, e quem dividir o aparelho não vê. O gate de banco
   (`/api/exercise-tutorial`) só serve os exercícios já convertidos.
4. **Duas histórias e seis famílias que ninguém alterou:**
   - **`d6` e `x4`, as duas do bolo**, que a auditoria sinalizou como **ambíguas** — continuam como
...
Lab `ordem-hist-a` (já removido), saída em `~/codex-lab/saida-ordem-hist-a-20260914.txt`.
**Veredito: ACEITA com 0 consertos.** O Codex não commitou.

14/09/2026 — **Ordem da História, Fatia B** — **Codex `gpt-5.6-sol`, esforço high** — motivo: a
segunda tentativa toca mecânica, registro clínico e tela ao mesmo tempo, com a regra dura de não
...
**Entregue nas versões v2.98.2 a v2.98.5** — commits `37a47fa`, `23ac56a`, `db66428`, `dee21e1`.
...
- [x] **Passo 2 — Codex `gpt-5.6-terra` (esforço high)** rodou no lab descartável
      `dupla-tarefa-ui`. Colheita gerada, revisada linha a linha pelo VP e o lab já foi removido. ✅
- [x] **Passo 3 — revisão + aplicação no repo vivo.** Commit `37a47fa` (v2.98.2), tocando
      `components/exercises/attention/DualTask.tsx` (+ `package.json` e os logs de pedidos).
      Na revisão o VP achou e consertou **4 defeitos da colheita**: ✅
      1. o painel principal recebia **duas classes de background em disputa** no mesmo elemento
         (`bg-white` e `bg-[#F6F8FB]`);
      2. o feedback do dígito **substituía o `pal.digitBox` inteiro** e levava junto a cor do
         texto — o número sumiria no tema escuro;
      3. **"Seu progresso" estava empilhado sobre a barra**, e no mockup dela fica ao lado;
      4. o **chip de nível tinha cor clara fixa** nos três temas.
- [x] **Passo 3a — v2.98.3: as formas ganharam acabamento (variante E).** Veio do uso dela:
      *"o desenho esta com baixa qualidade não é um detalhe é a qualidade do desenho como um
      todo"*. O agente `diretor` produziu **6 variantes** numa página de comparação e ela escolheu
      a **E**. O que mudou: ✅
      - **saiu o contorno preto de fora** — entra um filete da própria família de cor (−10 L*),
        recortado por `clipPath`;
      - **vértices arredondados** por arco tangente de raio 8 nas quatro formas;
      - **correção de PESO ÓTICO**: medida a área preenchida, o triângulo tinha **54,4** de
        "tinta" contra **75,6** do quadrado (**39% de dispersão**). Não é estética: dos níveis 8
        a 10 o alvo **alterna a cada bloco** justamente entre TRIÂNGULO VERDE e QUADRADO AZUL
        (`blockTarget`), então a saliência do alvo mudava onde a regra alterna — e essa acurácia
        alimenta a engine adaptativa. Agora a dispersão é de **12%**.
      - **Limite clínico respeitado:** o degradê anda só no eixo **L\*** do CIELAB (±3), matiz e
        croma preservados; desvio máximo **ΔE₀₀ 3,02** contra **ΔE₀₀ 19,7** do par de cores mais
        próximo do exercício. Commit `23ac56a`.
      - ⚠️ **Pendência não medida:** o custo do filtro de sombra (`feDropShadow`) num elemento que
        o framer-motion **reanima a cada 1,6–2,6 s**.
- [x] **Passo 3b — v2.98.4: saiu a linha "Ritmo" das instruções.** Decisão dela vendo a tela:
...
- [x] **Passo 3c — v2.98.5: a omissão deixa de ser avisada, e o voltar sai.** Ela, vendo a tela:
      *"tirei esse rologio, nao precisa avisar é treino e tire o botao voltar"*. O "relógio" desta
      ...
      - 📌 **Regra que vale além deste exercício:** *o app não comenta o que o paciente deixou de
        fazer.* Virou tarefa própria — ver o bloco seguinte (auditoria + 6 pontos corrigidos).
- [x] **Passo 4 — prova rodada em 31/ago/2026.** ✅
      ...
      - `grep -n "SUPERIOR\|INFERIOR\|Em cima\|Embaixo\|N-back" components/exercises/attention/DualTask.tsx`
        → **exit 1, nenhuma linha** (os cinco rótulos não existem mais no arquivo).
      - `npm run build` **não foi rodado** de propósito: o dev server dela está no ar na porta 3000
        (lição de 28/ago).
- [x] **Passo 5 — bump + commits feitos.** **v2.98.2**, **v2.98.3**, **v2.98.4** e **v2.98.5**
      commitadas (`37a47fa`, `23ac56a`, `db66428`, `dee21e1`). ✅ **Publicação segue pendente** —
      está contabilizada nas *Pendências reais* do bloco das Torres, mais abaixo.
- [ ] **Passo 6 — verificação visual dela**, desktop e celular. *Critério:* ela confirma com os
      olhos nos dois formatos. **PENDENTE — única pendência deste bloco.**

### Decisões abertas com ela

1. **Publicar ou não.** Segue aberta e cresceu: a contagem atual está nas *Pendências reais* do
   bloco das Torres. A produção está atrás de **toda a migração ao palco**, Cubo Corsi, MOT,
   Vigilância, os 7 de memória, a Busca Rápida, a Dupla Tarefa, os avisos de omissão e a Torre.
...
3. **O amarelo com 1,82:1 de contraste** contra o fundo claro — dívida **preexistente**, não
   ...
   cor**, merece tarefa própria. **Segue aberta.**
...
30-31/ago/2026 — **Dupla Tarefa (acabamento das formas, variante E)** — **agente `diretor`
(Opus 5, xhigh)** — motivo: decisão de desenho com limite clínico (cor, peso ótico e ΔE), classe
que não se delega ao lab.

## ✅ CONCLUÍDO — A regra do aviso passivo aplicada em 6 pontos (31/ago/2026)

**A regra, nas palavras dela:** *"tirei esse rologio, nao precisa avisar é treino"* — **o app não
comenta o que o paciente deixou de fazer.** É a irmã da regra do erro fechada em 28/ago (sai a
MENSAGEM, o erro continua visível), agora estendida ao **aviso passivo**: sai o carimbo da omissão,
**nunca a medida**. Registrada na memória `principio-sem-dica-apos-instrucao`.

**Auditoria que originou o lote:** `docs/auditoria-aviso-omissao/AUDITORIA-AVISO-OMISSAO-2026-08-31.md`
— **65 arquivos varridos**, commit `0d8ae6e`.

- [x] **Semáforo — v2.99.0, commit `10a5050`. O mais grave, e não era estética.** ✅
      O timeout chamava **o mesmo caminho do botão PARAR**. Consequência clínica: com **sinal
      vermelho**, não fazer nada era contado como **ACERTO** e a tela ainda mostrava **"Certo!"**.
      Com a distribuição de **45% verde / 45% vermelho / 10% amarelo**, um paciente **inerte**
      acertaria **55% da sessão** — e esses falsos acertos entravam no **tempo médio de reação**
      com o tempo cheio do timeout, contaminando a métrica.
      Corrigido extraindo função pura em **`lib/semaforo.ts`** com teste próprio, e provado
      **POR INJEÇÃO**: reintroduzido o bug, **3 testes quebram**.
- [x] **Lote 1 — v2.99.1, commit `43b42b3`.** ✅
      - **Focus Agentes, em dois pontos** — a faixa **"Acabou o tempo!"** ainda **AMPLIAVA o alvo
        não tocado**, ou seja, apontava com o dedo o que ele deixou de fazer;
      - **Busca Rápida** — saiu **só a origem por tempo**; coletar tudo continua mostrando o
        fechamento (é resposta a um gesto dela, não a uma ausência);
      - **Tempo de Reação** — o flash da omissão era **idêntico ao do toque errado**; além de sair
        o aviso, **a omissão passou a ser medida**, o que não existia.
- [x] **Labirinto — v2.99.2, commit `b1f42af`.** ✅ Saem o **"⏰ Tempo!"** e a frase
      *"Tente planejar a rota antes de andar"*. **O painel e as métricas ficam.**
- [x] **Compra Multifuncional — v2.99.3, commit `a777c5b`. O pior do lote.** ✅
      O cronômetro **CONFIRMAVA uma resposta que o paciente não deu** e ainda **revelava a conta
...
**Princípio dela que rege tudo:** *"o paciente precisa resolver o problema, e não jogar contra o
placar"*.

1. [x] **Fatia 1 — v3.1.0 (`9da4544`).** O **placar (contador de movimentos e mínimo) sai da
        execução**; a tela de conclusão **informa em vez de cobrar**; **resolver** vira o sucesso;
        progressão por **eficiência ≤ 1,40**; teto de **8 para 5 discos**; limite de reinícios cai.
        ⚠️ **Conserto achado na revisão do VP:** a `accuracy` ia virar *"resolvidos ÷ total"* e
        daria **100% em toda sessão** — e `accuracy` **alimenta a engine adaptativa**
        (`lib/adaptive.ts:154`, **≥ 0,80 sobe de nível**), então a Torre **subiria de nível para
        sempre** e ainda **dispararia sozinha a conquista de 100%**. Passou a ser a **proporção de
        resolvidos com eficiência boa ou adequada** (`TorreHanoi.tsx`, `eficientes / total`).
2. [x] **Fatia 3 — v3.1.1 (`c76c940`).** O **reinício vira evento** (com movimento e tempo),
        **movimentos totais × os da solução final**, **latência até o primeiro movimento**,
        **movimentos inválidos** com a mensagem dela, e **reversões** calculadas por **função pura
        testada**.
3. [x] **Fatia 4 — v3.2.0 (`70b5311`).** **Segunda tentativa opcional**, **uma só por problema**,
        com comparação **sem elogio e sem crítica**. A **PRIMEIRA** tentativa é que alimenta
        **progressão e acurácia** — a segunda acontece já sabendo que existe caminho melhor.
4. [x] **Fatia 5 motor — v3.3.0 (`54739df`).** **`lib/torres/`** com **BFS exata sobre os 3^n
        estados** e **banco de 40 problemas pré-validados** (**5 com 3 discos, 18 com 4, 12 com 5,
        5 com 6** — os de 6 **todos na fase 6**; **nenhum com 7 ou 8**). **Prova que sustenta
        tudo:** a BFS bate **exatamente com 2^n−1 de 1 a 6 discos** (`lib/torres/minimo.test.ts`).
5. [x] **Fatia 5b — v3.4.0 (`de456f5`).** Os **problemas variados chegam à tela**. Vitória deixa
        de ser *"haste 2 cheia"* e vira **igualdade com o alvo**; o **mínimo vem do problema**; a
        progressão sobe de **FASE**; e a **tela de objetivo com miniatura** fica **visível** nos
        alvos arbitrários (**seção 47 dela**: não pode virar jogo de memória).
6. [x] **Fatia 6 — v3.5.0 (`88b60fd`).** Tabela **`ExerciseAttempt`**: o **abandono deixa de ser
        invisível**. **MUDANÇA NO BANCO DE PRODUÇÃO com o rito cumprido** — backup em
        `~/neuropeak-backups/neuropeak-20260901-003324.dump` (**313 KB**), validado com
        `pg_restore --list` (**580 entradas**, as **7 tabelas do app** conferidas), classificado
        **nível 1**, e as **3 CHECK de `Session`** conferidas **ANTES e DEPOIS** do push — **as três
        sobreviveram**.
7. [~] **Fatia 7 — v3.6.0 (`45e91f5`): PARCIAL.** Entregue o painel **"Tentativas iniciadas"**
        (iniciados / concluídos / abandonados por exercício) na **aba de sessões do paciente**.
        **NÃO entregue** o **detalhe por desafio da seção 56**: os dados **já são gravados desde a
        fatia 3**, mas **não há UMA sessão de `torre-hanoi` no banco** — seria uma tela que
...
- **Codex `gpt-5.6-terra` (high)** — **fatias 1 e 3** das Torres e o **lote 1 dos avisos de
  omissão**; classe "código estruturado e testável" com spec escrita antes.
- **Codex `gpt-5.6-sol` (xhigh)** — **Compra Multifuncional** (aviso de omissão) e o **motor BFS**
