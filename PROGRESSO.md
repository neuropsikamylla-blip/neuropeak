...
2. **Decisões de produto/migração:** C2 🟡 CÓDIGO FEITO (pinPlain removido do schema, das 2 rotas que gravavam, da página do terapeuta e da prop de PatientCredentials; `prisma generate` local; tsc+build exit 0). ⏳ FALTA a parte destrutiva no banco: **a Kamylla** precisa rodar `npx prisma db push --accept-data-loss` no Supabase de produção (drop da coluna) — DEPOIS do deploy do código novo. Enquanto não rodar, os pinPlain ANTIGOS continuam no banco (coluna órfã). SEC-04 (CRP gate server-side) e A1-completo (score server-side) seguem pendentes.
