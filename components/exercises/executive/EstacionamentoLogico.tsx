  ...
  useLayoutEffect(() => {
    const compute = () => {
      ...
      const cromo = conteudo && tabuleiro
        ...
        : 0;
      // O TOPO entra na conta, não só o cromo interno: o palco tem padding próprio e o bloco não
      // começa no pixel zero da janela. Sem isto a altura fica otimista em ~30px e, em celular na
      // HORIZONTAL (852×393 medido), o tabuleiro estourava a dobra em 29px — que é exatamente a
      // reclamação dela de 11/ago/2026 no MOT, o botão caindo abaixo do visível.
      const topo = Math.max(0, conteudo?.getBoundingClientRect().top ?? 0);
      const altura = window.innerHeight - topo - cromo - RESPIRO_VERTICAL;
