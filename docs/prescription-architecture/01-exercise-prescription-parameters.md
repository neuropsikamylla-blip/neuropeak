# Parâmetros de prescrição dos exercícios

## Escopo e regras de leitura

Este documento prescreve os 34 exercícios canônicos sem alterar catálogo, mecânica, níveis, perfis cognitivos, modalidades ou engine. Os parâmetros seguem a mecânica congelada da Fase 1. A carga cognitiva é apenas basal, para configuração padrão em nível médio; modificadores são qualitativos e não têm pesos. Duração não foi convertida automaticamente em fadiga, e tempo de tela não foi convertido em interferência.

`CONTINUOUS_TIMED` encerra na próxima unidade discreta sem perder protocolo; `CLOSED_PROTOCOL` exige fechar a unidade usada pela progressão; `PLANNING_WINDOW` preserva o trabalho de resolução até o desafio terminar; `FIXED_HIGH_FATIGUE` usa exposição fixa porque alongá-la degrada o dado. Os tempos são estimativas clínicas, não mudanças no código.

> **Caminhos para a Meta:** todos os parâmetros da ficha são provisórios (`PROVISIONAL_PARAMETERS`) e não podem servir de referência para outros exercícios.

## Fichas

### 1. Span Numérico Auditivo Direto

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A extensão adapta somente após reprodução integral, e a sessão é prescrita por quantidade de tentativas.

2. **Unidade mínima válida — série:** Apresentação e reprodução completas são necessárias para interpretar retenção serial.

3. **Política de encerramento:** ao atingir o limite, não inicia outra série. **Limite no meio da unidade:** Se o limite chegar durante apresentação ou resposta, conclui a série; uma interrupção real invalida apenas a série parcial.
...
10. **Retomada após interrupção — do início do bloco:** Preserva séries fechadas, descarta a interrompida e reinicia o conjunto restante.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **início ou meio; no fim apenas em BREVE**. **Combinações ruins:** Span Numérico Auditivo Inverso: Interferência de ordem direta/inversa. Letras em Sequência: Fadiga verbal e efeito de ensaio serial.

12. **Modalidade:** não se aplica

### 2. Cores e Palavras

1. **Modelo de execução — `CONTINUOUS_TIMED`:** Cada tentativa fecha em poucos segundos e o temporizador é verificado ao final dela; não há série obrigatória.

2. **Unidade mínima válida — tentativa:** Regra, estímulo conflitante e escolha fechada já produzem um resultado interpretável.

3. **Política de encerramento:** ao atingir o limite, termina a tentativa. **Limite no meio da unidade:** Se o limite chegar com a barra ativa, permite resposta ou omissão e fecha a tentativa; não apresenta outro estímulo.
...
10. **Retomada após interrupção — retoma exatamente de onde parou:** Mantém tentativas fechadas; descarta a incompleta e inicia novo estímulo.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início, após adaptação curta ao dispositivo**. **Combinações ruins:** Alternância de Regras: Somação de troca e inibição reduz a comparabilidade. Semáforo: Mapeamentos de cor e resposta geram interferência proativa.

12. **Modalidade:** não se aplica

### 3. Agentes Focus

1. **Modelo de execução — `CONTINUOUS_TIMED`:** Cada comando e busca formam rodada autônoma, e o tempo é verificado ao final dela.

2. **Unidade mínima válida — rodada:** Apresentar e ocultar o comando, buscar e selecionar são necessários para interpretar manutenção do critério.

3. **Política de encerramento:** ao atingir o limite, termina a rodada. **Limite no meio da unidade:** Se o limite chegar durante a busca, fecha a janela do alvo e a rodada; não inicia novo comando.
...
10. **Retomada após interrupção — retoma exatamente de onde parou:** Preserva rodadas completas; descarta a cortada e reinicia com novo comando.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **início ou meio; no fim apenas em BREVE**. **Combinações ruins:** Rastreamento de Objetos: Rastreamento e fadiga em campo móvel se acumulam. Vigilância: Concentra discriminação visual e controle de distração.

12. **Modalidade:** Aplicável. **Estado:** projeção; seletor aprovado e ainda não implementado. **Visual:** duração referência; carga — Comando escrito e amostras visuais apoiam codificação. **Visual + áudio:** duração neutro ou até ~10% menor se a fala facilitar; carga — Redundância congruente pode facilitar; só aumenta divisão/interferência se fontes competirem. Não é automaticamente mais difícil. **Somente áudio:** duração ~15–25% maior; carga — Aumenta memória verbal auditiva e conversão auditivo-visual; reduz pistas visuais do comando.

### 4. Span Numérico Auditivo Inverso

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A progressão depende da reprodução integral da série transformada; resposta parcial perde a operação central.

2. **Unidade mínima válida — série:** Codificação, inversão e reprodução completas são inseparáveis.

3. **Política de encerramento:** ao atingir o limite, não inicia outra série. **Limite no meio da unidade:** Se o limite chegar durante apresentação ou resposta, conclui a série; interrupção real invalida só a série parcial.
...
10. **Retomada após interrupção — do início do bloco:** Conserva séries completas e reinicia integralmente a série interrompida.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início**. **Combinações ruins:** Span Numérico Auditivo Direto: Interferência de regra direta/inversa. N-Back: Sobrecarga atualização e memória verbal.

12. **Modalidade:** não se aplica

### 5. Matriz Espacial

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A extensão ajusta somente após reprodução completa da sequência espacial.

2. **Unidade mínima válida — série:** Apresentação serial e reprodução integral do caminho são a menor medida válida.

3. **Política de encerramento:** ao atingir o limite, não inicia outra série. **Limite no meio da unidade:** Se o limite chegar durante flashes ou resposta, conclui a série; interrupção real invalida só a série parcial.
...
10. **Retomada após interrupção — do início do bloco:** Preserva séries fechadas e reinicia a interrompida desde o primeiro flash.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **início ou meio**. **Combinações ruins:** Matriz Espacial Inversa: Mistura regras direta e inversa. Cubos: Acumula retenção serial espacial.

12. **Modalidade:** não se aplica

### 6. Matriz Espacial Inversa

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A progressão exige fechar a sequência e sua reprodução invertida.

2. **Unidade mínima válida — série:** Codificação, inversão e reprodução espacial completas são inseparáveis.

3. **Política de encerramento:** ao atingir o limite, não inicia outra série. **Limite no meio da unidade:** Se o limite chegar no meio, conclui a série; interrupção real descarta toda a série parcial.
...
10. **Retomada após interrupção — do início do bloco:** Reinicia integralmente a série interrompida e conserva apenas séries fechadas.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início**. **Combinações ruins:** Matriz Espacial: Contaminação entre regras direta e inversa. Matriz com Rotações: Acumula transformação espacial intensa.

12. **Modalidade:** não se aplica

### 7. Jogo da Memória

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A adaptação ocorre ao fechar o tabuleiro por sucesso ou orçamento de desencontros.

2. **Unidade mínima válida — rodada:** Somente tabuleiro encerrado permite interpretar aprendizagem, atualização e repetição de escolhas.

3. **Política de encerramento:** ao atingir o limite, termina a rodada. **Limite no meio da unidade:** Se o limite chegar no meio do tabuleiro, permite concluir a rodada e depois não inicia outra.
...
10. **Retomada após interrupção — do início do bloco:** Tabuleiro interrompido perde a história de exposições; reinicia a rodada.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **meio ou fim**. **Combinações ruins:** Matriz Espacial: Transfere estratégia de codificação de posições. Cubos: Acumula memória visuoespacial e fadiga perceptiva.

12. **Modalidade:** não se aplica

### 8. Conecta Números

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A sequência precisa chegar ao maior número para produzir resultado de rodada.

2. **Unidade mínima válida — rodada:** O caminho completo mostra cobertura, sequenciamento e erros fora de ordem.

3. **Política de encerramento:** ao atingir o limite, termina a rodada. **Limite no meio da unidade:** Se o limite chegar no meio, conclui o caminho em andamento e não gera nova disposição.
...
10. **Retomada após interrupção — do início do bloco:** Uma disposição interrompida reinicia para não preservar metade do caminho.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **início como aquecimento ou fim**. **Combinações ruins:** Busca Rápida: Prática de varredura e fadiga visual. Identificação de Símbolos: Repetição de busca reduz independência.

12. **Modalidade:** não se aplica

### 9. Caminhos para a Meta — PARÂMETROS PROVISÓRIOS

1. **Modelo de execução — `PLANNING_WINDOW`:** PROVISÓRIO: cortar a organização elimina o encadeamento construído e a relação plano–imprevisto; não usar como referência para outros exercícios.

2. **Unidade mínima válida — desafio completo:** PROVISÓRIO: a meta só é interpretável após estruturar e confirmar todas as ações e, se houver, o imprevisto.

3. **Política de encerramento:** ao atingir o limite, termina o desafio. **Limite no meio da unidade:** PROVISÓRIO: se o limite chegar no meio, preserva a janela até confirmação e eventual imprevisto; depois não inicia outra atividade.
...
10. **Retomada após interrupção — outra estratégia:** PROVISÓRIO: reinicia da meta e cartões originais, sem restaurar ordem parcial.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início ou meio; parâmetros provisórios**. **Combinações ruins:** Jogo das Torres: PROVISÓRIO: acumula organização e planejamento. Estacionamento Lógico: PROVISÓRIO: resolução planejada consecutiva contamina persistência.

12. **Modalidade:** Aplicável. **Estado:** PROVISÓRIO; projeção, sem seletor em três vias implementado. **Visual:** duração referência; carga — Texto e posição ficam consultáveis e reduzem memória interna. **Visual + áudio:** duração neutro ou até ~10% menor; carga — Fala congruente pode facilitar; só eleva divisão/interferência se fontes competirem. Não é automaticamente mais difícil. **Somente áudio:** duração ~20–35% maior; carga — Eleva processamento auditivo e memória verbal; depende de contrato acessível para ordenar ações sem texto.

### 10. Informação em Foco

1. **Modelo de execução — `CONTINUOUS_TIMED`:** Cada questão fecha antes da verificação do tempo; não há série obrigatória.

2. **Unidade mínima válida — tentativa:** Questão, pista eventual, resposta e feedback produzem decisão completa.

3. **Política de encerramento:** ao atingir o limite, termina a tentativa. **Limite no meio da unidade:** Se o limite chegar durante leitura, pista ou segunda chance, conclui a questão; não apresenta outra.
...
10. **Retomada após interrupção — retoma exatamente de onde parou:** Preserva questões encerradas; reinicia a interrompida sem pista ou resposta parcial.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **meio; pode abrir para observar leitura sem fadiga**. **Combinações ruins:** Compra Multifuncional: Acumula leitura funcional, critérios e decisões. Supermercado: Conteúdo de produtos gera familiaridade e interferência semântica.

12. **Modalidade:** não se aplica

### 11. Rastreamento de Objetos

1. **Modelo de execução — `FIXED_HIGH_FATIGUE`:** Rastreamento simultâneo em movimento é sensível à fadiga visual; a exposição deve ser fixa.

2. **Unidade mínima válida — rodada:** Codificação, movimento completo e identificação final medem preservação de identidade.

3. **Política de encerramento:** ao atingir o limite, termina a rodada. **Limite no meio da unidade:** Se o limite chegar durante movimento ou seleção, conclui a rodada; interrupção real invalida a parcial.
...
10. **Retomada após interrupção — um nível abaixo:** Se a interrupção refletir intolerância, reinicia bloco um nível abaixo; se externa, mantém o nível e reinicia a rodada.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início**. **Combinações ruins:** Agentes Focus: Movimento e busca acumulam fadiga perceptiva. Dupla Tarefa: Somação de atenção dividida reduz validade.

12. **Modalidade:** não se aplica

### 12. Dupla Tarefa

1. **Modelo de execução — `FIXED_HIGH_FATIGUE`:** Dois fluxos assíncronos ficam ativos sem pausas naturais; exposição longa degrada o dado.

2. **Unidade mínima válida — bloco:** Intervalo contínuo com ambos os fluxos é necessário para interpretar omissões e falsos positivos separados.

3. **Política de encerramento:** ao atingir o limite, termina imediatamente. **Limite no meio da unidade:** O fim programado encerra simultaneamente os fluxos e consolida o bloco; interrupção não programada invalida o bloco parcial.
...
10. **Retomada após interrupção — um nível abaixo:** Sobrecarga pede novo bloco integral um nível abaixo; interrupção externa, bloco integral no mesmo nível.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início, após breve checagem motora**. **Combinações ruins:** N-Back: Atualização contínua e prazo acumulam fadiga intensa. Rastreamento de Objetos: Atenção dividida consecutiva compromete tolerância.

12. **Modalidade:** não se aplica

### 13. Tempo de Reação

1. **Modelo de execução — `CONTINUOUS_TIMED`:** Cada leva fecha seus alvos e o resultado é registrado continuamente.

2. **Unidade mínima válida — rodada:** Uma leva completa reúne acertos, distratores e alvos perdidos sob mesma direção e velocidade.

3. **Política de encerramento:** ao atingir o limite, termina a rodada. **Limite no meio da unidade:** Se o limite chegar com balões em trânsito, deixa a leva resolver e não lança outra.
...
10. **Retomada após interrupção — retoma exatamente de onde parou:** Mantém levas fechadas; descarta a cortada e começa nova leva.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **início como aquecimento ou meio**. **Combinações ruins:** Semáforo: Resposta rápida a cor acumula preparação motora. Busca Rápida: Velocidade visual consecutiva aumenta fadiga.

12. **Modalidade:** não se aplica

### 14. Certo ou Errado

1. **Modelo de execução — `CONTINUOUS_TIMED`:** Cada situação recebe escolha, explicação e fechamento próprios.

2. **Unidade mínima válida — tentativa:** Afirmação, julgamento binário e feedback formam unidade interpretável.

3. **Política de encerramento:** ao atingir o limite, termina a tentativa. **Limite no meio da unidade:** Se o limite chegar durante leitura, permite responder e ver explicação; não mostra nova situação.
...
10. **Retomada após interrupção — retoma exatamente de onde parou:** Preserva itens fechados; reinicia situação interrompida antes da escolha.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **fim, com espaço para discussão**. **Combinações ruins:** Investigadores da Situação Social: Feedback normativo prepara respostas sociais subsequentes.

12. **Modalidade:** não se aplica

### 15. Semáforo

1. **Modelo de execução — `CONTINUOUS_TIMED`:** Cada sinal fecha por resposta ou fim da janela e o tempo global é verificado depois do feedback.

2. **Unidade mínima válida — tentativa:** Aviso, sinal-alvo e escolha avançar/parar produzem decisão completa.

3. **Política de encerramento:** ao atingir o limite, termina a tentativa. **Limite no meio da unidade:** Se o limite chegar com o sinal ativo, aguarda resposta ou fim da janela e não apresenta outro.
...
10. **Retomada após interrupção — retoma exatamente de onde parou:** Conserva tentativas completas; descarta o sinal interrompido e cria outro.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **início ou meio**. **Combinações ruins:** Cores e Palavras: Mapeamentos por cor geram interferência. Tempo de Reação: Aquecimento motor altera a segunda tarefa.

12. **Modalidade:** não se aplica

### 16. Busca Rápida

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A grade tem contador próprio e só fecha ao localizar alvos ou esgotar o tempo.

2. **Unidade mínima válida — rodada:** Regra, grade, conjunto de alvos e encerramento temporal precisam ser observados juntos.

3. **Política de encerramento:** ao atingir o limite, termina a rodada. **Limite no meio da unidade:** Se o limite global chegar durante a grade, deixa o contador terminar ou os alvos serem encontrados; não inicia outra.
...
10. **Retomada após interrupção — do início do bloco:** Grade parcial perde tempo e itens removidos; reinicia a rodada.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início ou meio**. **Combinações ruins:** Identificação de Símbolos: Busca em grade transfere varredura e fadiga. Agentes Focus: Concentra seleção visual sob distratores.

12. **Modalidade:** não se aplica

### 17. Jogo das Torres

1. **Modelo de execução — `PLANNING_WINDOW`:** Interromper antes da configuração final destrói a sequência de submetas e a medida de optimalidade.

2. **Unidade mínima válida — desafio completo:** Somente transferir toda a torre permite distinguir solução e número de movimentos.

3. **Política de encerramento:** ao atingir o limite, termina o desafio. **Limite no meio da unidade:** Se o limite chegar no meio, mantém a janela até a solução; interrupção real invalida o desafio parcial.
...
10. **Retomada após interrupção — outra estratégia:** Reinicia do estado inicial e registra nova trajetória, sem restaurar sequência parcial.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início**. **Combinações ruins:** Estacionamento Lógico: Duas janelas profundas de planejamento acumulam fadiga. Grade Dedutiva: Raciocínio prolongado reduz persistência.

12. **Modalidade:** não se aplica

### 18. Labirinto

1. **Modelo de execução — `PLANNING_WINDOW`:** Rota e pontos de decisão só são interpretáveis ao sair ou esgotar a janela própria.

2. **Unidade mínima válida — desafio completo:** Labirinto encerrado reúne solução, eficiência, becos, retornos e colisões.

3. **Política de encerramento:** ao atingir o limite, termina o desafio. **Limite no meio da unidade:** Se o limite global chegar durante a rota, permite saída ou fim da janela do labirinto; não gera outro mapa.
...
10. **Retomada após interrupção — outra estratégia:** Reinicia desde a origem sem conservar trilha parcial; se o mapa mudar, marca o anterior inválido.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início ou meio**. **Combinações ruins:** Estacionamento Lógico: Planejamento visuoespacial consecutivo acumula fadiga. Jogo das Torres: Duas janelas de resolução reduzem persistência.

12. **Modalidade:** não se aplica

### 19. Ordem da História

1. **Modelo de execução — `PLANNING_WINDOW`:** Cortar antes da confirmação perde relações temporal-causais construídas.

2. **Unidade mínima válida — desafio completo:** História confirmada é a menor unidade para julgar coerência, intruso ou lacuna.

3. **Política de encerramento:** ao atingir o limite, termina o desafio. **Limite no meio da unidade:** Se o limite chegar durante a ordenação, mantém a janela até confirmação e feedback; não apresenta outra história.
...
10. **Retomada após interrupção — outra estratégia:** Reinicia no embaralhamento inicial e descarta arranjo parcial.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **meio; pode fechar se a história for curta**. **Combinações ruins:** Caminhos para a Meta: Ordenação de cartões transfere estratégia. Grade Dedutiva: Raciocínio relacional consecutivo aumenta fadiga.

12. **Modalidade:** não se aplica

### 20. Compra Multifuncional

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A missão encadeia fases e ajusta nível pelos resultados; cada fase precisa fechar.

2. **Unidade mínima válida — fase:** Problema numérico ou combinatório completo, com confirmação e feedback, é a menor unidade válida.

3. **Política de encerramento:** ao atingir o limite, termina a fase. **Limite no meio da unidade:** Se o limite chegar durante cálculo, seleção ou nova tentativa, conclui a fase; não inicia a seguinte.
...
10. **Retomada após interrupção — do início do bloco:** Preserva fases fechadas e reinicia a interrompida com dados originais.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início ou meio, com tempo para fechar a fase**. **Combinações ruins:** Informação em Foco: Acumula leitura funcional e critérios. Grade Dedutiva: Sobrecarga integração verbal e resolução.

12. **Modalidade:** Aplicável. **Estado:** projeção; seletor aprovado e ainda não implementado. **Visual:** duração referência; carga — Enunciado, valores e regras ficam consultáveis. **Visual + áudio:** duração neutro a ~10% maior; pode diminuir se apoiar leitura; carga — Redundância pode facilitar; competição voz-inspeção pode elevar divisão. Não é automaticamente mais difícil. **Somente áudio:** duração ~20–30% maior; carga — Aumenta processamento auditivo e memória de valores/regras; seleção exige identificação acessível.

### 21. Alternância de Regras

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A operação depende de blocos e da primeira tentativa após trocas; tentativas soltas não medem alternância.

2. **Unidade mínima válida — bloco:** Quatro a seis cartas fecham manutenção da regra e preparam troca interpretável.

3. **Política de encerramento:** ao atingir o limite, termina o bloco. **Limite no meio da unidade:** Se o limite chegar no meio, conclui as cartas do bloco e não apresenta nova regra.
...
10. **Retomada após interrupção — do início do bloco:** Reapresenta a regra e reinicia o bloco, sem criar troca adicional por interrupção.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início**. **Combinações ruins:** Cores e Palavras: Troca e inibição compartilhadas geram interferência. Dupla Tarefa: Atualização após atenção dividida reduz validade.

12. **Modalidade:** não se aplica

### 22. Grade Dedutiva

1. **Modelo de execução — `PLANNING_WINDOW`:** A cadeia de pistas perde sentido se cortada antes da solução correta.

2. **Unidade mínima válida — desafio completo:** Grade resolvida permite interpretar integração, revisões e monitoramento.

3. **Política de encerramento:** ao atingir o limite, termina o desafio. **Limite no meio da unidade:** Se o limite chegar no meio, mantém a janela até solução e feedback; não seleciona outro problema.
...
10. **Retomada após interrupção — outra estratégia:** Reinicia com grade limpa e mesmas pistas, sem restaurar exclusões parciais.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início ou meio, com tempo para solução**. **Combinações ruins:** Jogo das Torres: Raciocínio prolongado acumula fadiga. Compra Multifuncional: Integração verbal sob regras sobrecarrega memória.

12. **Modalidade:** não se aplica

### 23. Letras em Sequência

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A resposta só é classificada após reconstruir toda a série; direção e canal podem mudar por nível.

2. **Unidade mínima válida — série:** Apresentação serial e reconstrução completa entre alvos/distratores formam medida válida.

3. **Política de encerramento:** ao atingir o limite, não inicia outra série. **Limite no meio da unidade:** Se o limite chegar durante apresentação ou resposta, conclui a série; interrupção real a invalida.
...
10. **Retomada após interrupção — do início do bloco:** Reinicia desde o primeiro item e mantém canal/direção prescritos.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **início ou meio**. **Combinações ruins:** Span Numérico Auditivo Direto: Efeito de ensaio e fadiga subvocal. Lista com Distração: Manutenção verbal contamina a tarefa seguinte.

12. **Modalidade:** não se aplica

### 24. Sequência de Itens

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A adaptação ocorre após reconstruir toda a série; resposta parcial não avalia identidade e ordem.

2. **Unidade mínima válida — série:** Codificação, ocultação e seleção ordenada completa são a menor unidade.
...
10. **Retomada após interrupção — do início do bloco:** Reinicia desde a codificação, preservando só séries fechadas e canal prescrito.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **meio**. **Combinações ruins:** Supermercado: Itens e listas geram interferência semântica. Jogo da Memória: Memória visual consecutiva acumula fadiga.

12. **Modalidade:** não se aplica

### 25. Matriz com Rotações

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A rodada só fecha após codificar, transformar e confirmar todo o padrão.

2. **Unidade mínima válida — série:** Padrão, rotação e reconstrução confirmada formam a menor medida.

3. **Política de encerramento:** ao atingir o limite, não inicia outra série. **Limite no meio da unidade:** Se o limite chegar durante flashes, giro ou resposta, conclui a série; interrupção real a invalida.
...
10. **Retomada após interrupção — do início do bloco:** Reinicia desde a apresentação do padrão e mantém só séries confirmadas.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início**. **Combinações ruins:** Matriz Espacial Inversa: Transformações espaciais acumulam fadiga. Cubos: Remapeamento compartilhado reduz independência.

12. **Modalidade:** não se aplica

### 26. Lista com Distração

1. **Modelo de execução — `CLOSED_PROTOCOL`:** Lista, atividade interveniente e recuperação precisam fechar juntas.

2. **Unidade mínima válida — rodada:** Memorização, interferência e recuperação formam a menor unidade clínica.

3. **Política de encerramento:** ao atingir o limite, termina a rodada. **Limite no meio da unidade:** Se o limite chegar em qualquer fase, conclui miniatividade e recuperação; interrupção real invalida a rodada.
...
10. **Retomada após interrupção — do início do bloco:** Reinicia desde a lista para não alterar intervalo de retenção.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início ou meio**. **Combinações ruins:** N-Back: Manutenção e atualização acumulam fadiga. Restaurante: Listas sob distratores sofrem interferência proativa.

12. **Modalidade:** não se aplica

### 27. Restaurante

1. **Modelo de execução — `CLOSED_PROTOCOL`:** Pedido, ocultação, montagem e confirmação precisam fechar para adaptar a rodada seguinte.

2. **Unidade mínima válida — rodada:** Pedido ligado à mesa, bandeja completa e feedback formam unidade interpretável.

3. **Política de encerramento:** ao atingir o limite, termina a rodada. **Limite no meio da unidade:** Se o limite chegar na memorização ou montagem, permite confirmar e receber feedback; não mostra outra mesa.
...
10. **Retomada após interrupção — do início do bloco:** Reinicia desde o pedido; retomar na bancada alteraria retenção e exposições.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **meio; no fim apenas em BREVE**. **Combinações ruins:** Supermercado: Listas semelhantes geram interferência de fonte. Lista com Distração: Manutenção sob distratores acumula interferência.

12. **Modalidade:** Aplicável. **Estado:** implementado; resposta visual em todos os modos. **Visual:** duração referência; carga — Foto e nome apoiam memória visual e recodificação. **Visual + áudio:** duração neutro ou até ~10% menor; carga — Redundância pode facilitar; só aumenta divisão se fala, texto ou ruído competirem. Não é automaticamente mais difícil. **Somente áudio:** duração ~10–20% maior, inclusive por repetição; carga — Aumenta memória auditiva, mas preserva cena, pessoas e resposta visual; não é auditiva pura.

### 28. Supermercado

1. **Modelo de execução — `CLOSED_PROTOCOL`:** Lista, ocultação, busca e confirmação precisam fechar; ordem e fonte só aparecem na compra completa.

2. **Unidade mínima válida — rodada:** Lista solicitada e carrinho confirmado são a menor unidade de recuperação sob busca.

3. **Política de encerramento:** ao atingir o limite, termina a rodada. **Limite no meio da unidade:** Se o limite chegar na memorização ou prateleira, permite confirmar e ver resultado; não apresenta nova lista.
...
10. **Retomada após interrupção — do início do bloco:** Reinicia desde a lista para preservar intervalo de retenção e exposições.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início ou meio**. **Combinações ruins:** Restaurante: Listas funcionais semelhantes geram interferência. Sequência de Itens: Nomeação e objetos favorecem interferência semântica.

12. **Modalidade:** Aplicável. **Estado:** implementado; fotos persistem em partes de Somente áudio. **Visual:** duração referência; carga — Fotos e nomes apoiam memória visual e recodificação. **Visual + áudio:** duração neutro ou até ~10% menor; carga — Redundância congruente pode facilitar e não é automaticamente mais difícil; fontes concorrentes podem dividir atenção. **Somente áudio:** duração ~10–20% maior, sobretudo com repetição; carga — Eleva memória auditiva e pareamento nome-imagem, mas fotos e alguns nomes permanecem.

### 29. N-Back

1. **Modelo de execução — `FIXED_HIGH_FATIGUE`:** Cada bloco tem preparação e 14 estímulos contínuos; progressão exige blocos completos e longa exposição fatiga atualização.

2. **Unidade mínima válida — bloco:** A sequência completa sob o mesmo N é necessária para decidir progressão.

3. **Política de encerramento:** ao atingir o limite, termina o bloco. **Limite no meio da unidade:** Se o limite chegar durante os 14 estímulos, conclui o bloco; interrupção real invalida o parcial.
...
10. **Retomada após interrupção — um nível abaixo:** Sobrecarga pede novo bloco um N abaixo; interrupção externa repete bloco integral no mesmo N.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início**. **Combinações ruins:** Dupla Tarefa: Atualização contínua e prazo acumulam fadiga intensa. Lista com Distração: Interferência verbal altera manutenção e descarte.

12. **Modalidade:** não se aplica

### 30. Cubos

1. **Modelo de execução — `CLOSED_PROTOCOL`:** A extensão adapta após reprodução completa; cortar perde retenção e remapeamento.

2. **Unidade mínima válida — série:** Giros, flashes e toques em ordem são necessários para interpretar memória visuoespacial.
...
10. **Retomada após interrupção — do início do bloco:** Reinicia desde o primeiro giro e preserva apenas séries completas.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início ou meio**. **Combinações ruins:** Matriz com Rotações: Remapeamento e rotação acumulam fadiga. Matriz Espacial: Retenção serial espacial favorece interferência.

12. **Modalidade:** não se aplica

### 31. Vigilância

1. **Modelo de execução — `FIXED_HIGH_FATIGUE`:** Exposições breves repetidas são sensíveis a fadiga perceptiva; blocos fixos preservam comparabilidade.

2. **Unidade mínima válida — bloco:** Seis tentativas completas oferecem repetição mínima do ciclo sem tratar uma imagem como vigilância.

3. **Política de encerramento:** ao atingir o limite, termina o bloco. **Limite no meio da unidade:** Se o limite chegar no meio, conclui a tentativa e o bloco fixo; interrupção real invalida o bloco parcial.
...
10. **Retomada após interrupção — um nível abaixo:** Fadiga visual pede novo bloco um nível abaixo; interrupção externa repete integralmente no mesmo nível.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início**. **Combinações ruins:** Identificação de Símbolos: Discriminação consecutiva acumula fadiga. Agentes Focus: Seleção visual concorrente reduz tolerância.

12. **Modalidade:** não se aplica

### 32. Identificação de Símbolos

1. **Modelo de execução — `CONTINUOUS_TIMED`:** Cada pareamento fecha no primeiro toque e o encerramento é verificado após a tentativa.

2. **Unidade mínima válida — tentativa:** Modelo, grade e primeira seleção produzem comparação completa.

3. **Política de encerramento:** ao atingir o limite, termina a tentativa. **Limite no meio da unidade:** Se o limite chegar durante busca, permite toque e feedback; não gera outro modelo. Interpretar a primeira tentativa com cautela.
...
10. **Retomada após interrupção — retoma exatamente de onde parou:** Mantém tentativas fechadas; descarta a interrompida e cria novo pareamento.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **início como aquecimento ou meio**. **Combinações ruins:** Busca Rápida: Busca em grade transfere varredura. Vigilância: Discriminação repetida reduz validade.

12. **Modalidade:** não se aplica

### 33. Estacionamento Lógico

1. **Modelo de execução — `PLANNING_WINDOW`:** Interromper perde a cadeia de desbloqueios e impede classificar solução e distância do ótimo.

2. **Unidade mínima válida — desafio completo:** Retirar o carro-alvo fecha estados e permite comparar movimentos com a solução mínima.

3. **Política de encerramento:** ao atingir o limite, termina o desafio. **Limite no meio da unidade:** Se o limite chegar no meio, mantém a janela até a saída; interrupção real invalida a trajetória parcial.
...
10. **Retomada após interrupção — outra estratégia:** Reinicia no estado inicial e registra nova trajetória, sem misturar execução guiada e autônoma.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **não**; melhor posição: **início**. **Combinações ruins:** Jogo das Torres: Planejamento profundo consecutivo acumula fadiga. Labirinto: Resolução visuoespacial reduz persistência.

12. **Modalidade:** não se aplica

### 34. Investigadores da Situação Social

1. **Modelo de execução — `CLOSED_PROTOCOL`:** Perguntas da cena compartilham contexto e feedback; fechar o caso evita corte e contaminação desigual.

2. **Unidade mínima válida — desafio completo:** Caso social com todas as perguntas preserva contexto e sequência de feedback.

3. **Política de encerramento:** ao atingir o limite, termina o desafio. **Limite no meio da unidade:** Se o limite chegar entre perguntas, conclui o caso e encerra ao fim da cena; não inicia outra história.
...
10. **Retomada após interrupção — do início do bloco:** Reinicia o caso desde a primeira pergunta para não retomar após feedback parcial.

11. **Elegibilidade na sessão:** pode abrir: **sim**; pode fechar: **sim**; melhor posição: **fim, com espaço para discussão clínica**. **Combinações ruins:** Certo ou Errado: Feedback normativo prepara julgamentos socialmente desejáveis.

12. **Modalidade:** não se aplica

## Tabela-resumo

| Exercício | Modelo | Unidade mínima | Carga basal | Duração padrão | Fadiga | Interferência |
|---|---|---|---:|---:|---|---|
| Span Numérico Auditivo Direto | `CLOSED_PROTOCOL` | série | 2 | 6 min | moderada | baixa |
| Cores e Palavras | `CONTINUOUS_TIMED` | tentativa | 3 | 5 min | alta | alta |
| Agentes Focus | `CONTINUOUS_TIMED` | rodada | 2 | 6 min | moderada | moderada |
| Span Numérico Auditivo Inverso | `CLOSED_PROTOCOL` | série | 3 | 8 min | alta | baixa |
| Matriz Espacial | `CLOSED_PROTOCOL` | série | 2 | 6 min | moderada | baixa |
| Matriz Espacial Inversa | `CLOSED_PROTOCOL` | série | 3 | 8 min | alta | baixa |
| Jogo da Memória | `CLOSED_PROTOCOL` | rodada | 2 | 7 min | moderada | moderada |
| Conecta Números | `CLOSED_PROTOCOL` | rodada | 2 | 6 min | baixa | baixa |
| Caminhos para a Meta (provisório) | `PLANNING_WINDOW` | desafio completo | 2 | 9 min | moderada | baixa |
| Informação em Foco | `CONTINUOUS_TIMED` | tentativa | 2 | 10 min | moderada | moderada |
| Rastreamento de Objetos | `FIXED_HIGH_FATIGUE` | rodada | 3 | 6 min | alta | alta |
| Dupla Tarefa | `FIXED_HIGH_FATIGUE` | bloco | 3 | 6 min | alta | alta |
| Tempo de Reação | `CONTINUOUS_TIMED` | rodada | 1 | 5 min | moderada | moderada |
| Certo ou Errado | `CONTINUOUS_TIMED` | tentativa | 1 | 7 min | baixa | baixa |
| Semáforo | `CONTINUOUS_TIMED` | tentativa | 2 | 6 min | moderada | moderada |
| Busca Rápida | `CLOSED_PROTOCOL` | rodada | 3 | 8 min | alta | alta |
| Jogo das Torres | `PLANNING_WINDOW` | desafio completo | 3 | 9 min | alta | baixa |
| Labirinto | `PLANNING_WINDOW` | desafio completo | 3 | 9 min | moderada | moderada |
| Ordem da História | `PLANNING_WINDOW` | desafio completo | 2 | 9 min | moderada | moderada |
| Compra Multifuncional | `CLOSED_PROTOCOL` | fase | 3 | 18 min | alta | moderada |
| Alternância de Regras | `CLOSED_PROTOCOL` | bloco | 3 | 7 min | alta | alta |
| Grade Dedutiva | `PLANNING_WINDOW` | desafio completo | 3 | 13 min | alta | moderada |
| Letras em Sequência | `CLOSED_PROTOCOL` | série | 2 | 6 min | moderada | moderada |
| Sequência de Itens | `CLOSED_PROTOCOL` | série | 2 | 8 min | moderada | moderada |
| Matriz com Rotações | `CLOSED_PROTOCOL` | série | 3 | 8 min | alta | baixa |
| Lista com Distração | `CLOSED_PROTOCOL` | rodada | 3 | 9 min | alta | alta |
| Restaurante | `CLOSED_PROTOCOL` | rodada | 2 | 10 min | moderada | moderada |
| Supermercado | `CLOSED_PROTOCOL` | rodada | 3 | 12 min | alta | alta |
| N-Back | `FIXED_HIGH_FATIGUE` | bloco | 3 | 7,5 min | alta | alta |
| Cubos | `CLOSED_PROTOCOL` | série | 3 | 8 min | alta | baixa |
| Vigilância | `FIXED_HIGH_FATIGUE` | bloco | 3 | 6 min | alta | alta |
| Identificação de Símbolos | `CONTINUOUS_TIMED` | tentativa | 2 | 5 min | moderada | moderada |
| Estacionamento Lógico | `PLANNING_WINDOW` | desafio completo | 3 | 11 min | alta | moderada |
| Investigadores da Situação Social | `CLOSED_PROTOCOL` | desafio completo | 2 | 9 min | moderada | moderada |
