...
function normalizeDose(value: unknown): PrescribedDose | undefined {
  ...
  const dose = value as { kind?: unknown; protocol?: unknown; prescribedMinutes?: unknown; maximumMinutes?: unknown; minutes?: unknown };
 ...
 * `settings`, inclusive `trials` dos spans, é preservado em clinicalParameters; ausência de dose
 * resolve para PADRAO no interpretador. IDs desconhecidos nunca lançam e ficam registrados.
 */
export function readLegacyPlan(rawPlan: unknown, fallbackTargetMinutes: TargetMinutes = 20): LegacyReadResult {
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    if (!catalogExercise(exerciseId)) { ignoredIds.push(rawId); continue; }
    const settings = object?.settings && typeof object.settings === "object" ? object.settings as Record<string, unknown> : undefined;
    const clinical = object?.clinicalParameters && typeof object.clinicalParameters === "object" ? object.clinicalParameters as Record<string, unknown> : settings;
    exercises.push({
      exerciseId,
      order: typeof object?.order === "number" ? object.order : index + 1,
      dose: normalizeDose(object?.dose ?? settings?.protocol),
